...
export default function (globalListener: FlatfileListener): void {
  ...
  if (
    ...
    process.env.FLATFILE_BEARER_TOKEN.indexOf(".") > -1
  ) {
    try {
      const [_header, payload, ..._signature] =
        process.env.FLATFILE_BEARER_TOKEN!.split(".");
      const jwt = JSON.parse(Buffer.from(payload!, "base64").toString());

      process.env.FLATFILE_ENVIRONMENT_ID ||= jwt.env;
    } catch (_error) {
      throw new Error(
        `Missing \`FLATFILE_ENVIRONMENT_ID\` environment variable.`
      );
    }
  }
  ...
  if (!process.env.FLATFILE_EVENT_NAMESPACE) {
    console.warn(
      "Warning: `FLATFILE_EVENT_NAMESPACE` is not set. Listener will recieve account-wide events."
    );
  }
